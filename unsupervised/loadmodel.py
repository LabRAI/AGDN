...
def test(loader, topk=20):
    ...
    for batch in loader:
        ...
        sol = batch[2]
        t0 = time.time()
        output = model(xy_pos, distance_m)
        t1 = time.time()
...
# TSP200
# topk = 10
model_name = f'Saved_Models/TSP_{args.num_of_nodes}/{args.model}_layer_{args.nlayers}_hid_{args.hidden}_210_temp_3.5.pth'  # saved model
# generalization fix model
# model_name = f'Saved_Models/TSP_100/{args.model}_layer_{args.nlayers}_hid_{args.hidden}_210_temp_3.5.pth'  # saved model
model.load_state_dict(torch.load(model_name))
# Saved_indices,Saved_Values,Saved_sol,Saved_pos = test(test_loader,topk = 8) # epoch=20>10
print('Using topk: ', args.topk)
...
C = Saved_indices
V = Saved_Values
inference_name = f"1kTraning_{args.model}_TSP{args.num_of_nodes}Instance_{Saved_indices.shape[0]}.txt"
with open(inference_name, "w") as f:
