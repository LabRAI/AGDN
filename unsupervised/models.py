...
class SCTConv(torch.nn.Module):
    ...
    def forward(self, X, adj, moment=1, device='cuda'):
        ...
        a_input = torch.cat(
            (a_input_A, a_input_A2, a_input_sct1, a_input_sct2, a_input_sct3, a_input_sct4),
            1).view(B, 6, N, -1)
...
class GNN(nn.Module):
    ...
    def forward(self, X, adj, moment=1, device='cuda'):
        adj = preprocess_dist(adj, temperature=3.5)
        X = self.in_proj(X)
        hidden_states = X
...
def preprocess_dist(dist_matrix, temperature):
    # build mask
    num_of_nodes = dist_matrix.size(1)
    mask = torch.ones(num_of_nodes, num_of_nodes).cuda()
    mask.fill_diagonal_(0)
    # re-weight
    adj = torch.exp(-1. * dist_matrix / temperature)
    # mask self-loop
    adj *= mask
    return adj
