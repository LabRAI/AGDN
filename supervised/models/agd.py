...
class AGDLayer_Iso(nn.Module):
    def __init__(self, hidden_dim, K=10, alpha=0.1):
        super().__init__()
        print('AGDLayer Isomophic')
        ...
        self.param_ppr = Parameter(torch.Tensor(K + 1, 1))
        self.reset_parameters()

    def reset_parameters(self):
        # init by PPR
        for k in range(self.K + 1):
            self.param_ppr.data[k] = self.alpha * (1 - self.alpha) ** k
        self.param_ppr.data[-1] = (1 - self.alpha) ** self.K

    def prop_h(self, h, T):
        h = h * (self.param_ppr[0])
        h_ = h
        for k in range(self.K):
            h_ = F.relu(self.filter(torch.bmm(T, h_)))
            h = h + self.param_ppr[k + 1] * h_
        return h
    ...
    def aggr_h(self, h):
        ...
        hj = torch.einsum('bij,bjc->bic', attn, h)  # Shape: [batch, node, out_channels]
        # aggr h
        h_prime = self.lin_h(torch.cat([h, hj], dim=-1))
        return h_prime
...
class AGDLayer_Node(nn.Module):
    ...
    def prop_h(self, hi, hj, Ti, Tj):
        ...
        hj_ = hj
        for k in range(self.K):
            hi_ = F.relu(self.filter_i(torch.bmm(Ti, hi_)))  # activate
    ...
    def forward(self, x, e, T):
        ...
        # return h, e
        # return h_prime, None, Ti
        return h_prime, None


class AGDLayer_Edge(nn.Module):
    def __init__(self, hidden_dim, K=10, alpha=0.1):
        super().__init__()
        print('AGDLayer Edge')
        self.hidden_dim = hidden_dim
...
class Encoder_AGD(nn.Module):
    def __init__(self, encoder_layer_num, hidden_dim, **cfg):
        ...
        self.transition_norm = cfg['transition_norm']
        self.K = cfg['K']
        self.teleport_ratio = cfg['teleport_ratio']
        ...
        layer_dict = {'iso': AGDLayer_Iso, 'node': AGDLayer_Node, 'edge': AGDLayer_Edge}
        self.layers = nn.ModuleList([
            layer_dict[self.layer_name](hidden_dim, K=self.K, alpha=self.teleport_ratio) for _ in
            range(encoder_layer_num)
        ])
        self.lin_node1 = nn.Linear(hidden_dim * (1 + encoder_layer_num), hidden_dim)
        self.lin_node2 = nn.Linear(hidden_dim, hidden_dim)
        self.norm = nn.BatchNorm1d(hidden_dim, affine=True)
    ...
    def forward_iso(self, x, dist_matrix):
        ...
        h = self.lin_node1(h)
        h = self.norm(h.view(-1, self.hidden_dim)).view(h.shape)
        h = F.relu(h)
        h = self.lin_node2(h)
        hm = self.node_to_heatmap(h)
        hm = self.node_pred(hm)
        return hm

    def forward_node(self, x, dist_matrix):
        T = get_transition_matrix(dist_matrix, temperature=self.temperature,
                                  sparsify_ratio=self.sparse_ratio, norm=self.transition_norm)
        h = self.node_embed(self.node_pos_embed(x))
        e = None
    ...
    def forward_edge(self, x, dist_matrix):
        T = get_transition_matrix(dist_matrix, temperature=self.temperature,
                                  sparsify_ratio=self.sparse_ratio, norm=self.transition_norm)
        h = self.node_embed(self.node_pos_embed(x))
    ...
    def forward(self, x, dist_matrix):
        if self.layer_name == 'iso':
            hm = self.forward_iso(x, dist_matrix)
        elif self.layer_name == 'node':
            hm = self.forward_node(x, dist_matrix)
        elif self.layer_name == 'edge':
            hm = self.forward_edge(x, dist_matrix)
        else:
